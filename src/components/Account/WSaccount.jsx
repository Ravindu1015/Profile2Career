...
function WSaccount() {
  ...
  useEffect(() => {
    const fetchUserData = async (user) => {
      try {
        if (user) {
          const userDocRef = doc(db, 'whiteseekers', user.uid);
          ...
          navigate('/whitelogin'); // Redirect to WhiteLoginpage
        }
      } catch (error) {
  ...
  const handleSignOut = async () => {
    try {
      await signOut(auth);
      navigate('/whitelogin'); // Redirect to WhiteLoginpage after sign-out
    } catch (error) {
  ...
  const handleSaveChanges = async () => {
    try {
      const userDocRef = doc(db, 'whiteseekers', auth.currentUser.uid);
