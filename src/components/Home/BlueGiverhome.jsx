    ...
    const fetchUserDataAndPosts = async (user) => {
      try {
        if (user) {
          ...
          const postsQuery = query(collection(db, 'bgpost'));
          ...
          <div className="relative flex items-center justify-between h-16">
            ...
            <div className="flex-1 flex items-center justify-end">
              <div className="flex space-x-4">
                <NavLink to="/bluegivershome" className={({ isActive }) => isActive ? activeLinkClass : defaultLinkClass}>Home</NavLink>
                ...
                <NavLink to="/bluegivershelp" className={({ isActive }) => isActive ? activeLinkClass : defaultLinkClass}>Help</NavLink>
                <NavLink to="/bluegiversaccount" className={({ isActive }) => isActive ? activeLinkClass : defaultLinkClass}>
          ...
          {posts.length > 0 ? (
            posts.map((post) => (
              <div key={post.id} className="bg-gray-50 p-4 rounded-lg shadow-md mb-4 shadow-blue-300">
                ...
                {/* Post Image */}
                {post.bgimage && (
                  <img src={post.bgimage} alt="Job" className="w-full h-40 object-cover mb-4 rounded-lg" />
                )}

                {/* Post Details */}
                <p className="text-gray-700 mb-4">{post.bgmoreinfo?.join(', ') || 'No Additional Info'}</p>
