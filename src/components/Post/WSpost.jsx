...
function WSpost() {
  ...
  const [havingSkills, setHavingSkills] = useState(['']);
  const [salary, setSalary] = useState('');
  const [moreInfo, setMoreInfo] = useState(['']);
  const [image, setImage] = useState(null);

  const navClass = theme === 'white' ? 'bg-gray-100 text-gray-800' : 'bg-white text-gray-800';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'wspost'), {
        wsjobtitle: jobTitle,
        wshavingskills: havingSkills,
        wssallary: salary,
        wsmore: moreInfo,
        wsimage: image ? URL.createObjectURL(image) : '' // Convert image to a URL string
      });
    ...
    <div className="bg-white h-screen flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-3 gap-4 p-4 mt-16">
        ...
        <div className="sticky top-16 bg-gray-100 p-6 rounded-xl shadow-black shadow-lg col-span-1">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-gray-600 mb-6">
              You are looking to acquire new skills and explore opportunities for personal and professional growth. 
            </p>
            <hr className="w-2/3 border-t border-gray-300 mb-6" />
            <p className="text-gray-600 mb-6">
              Post job skills that you are seeking to improve on to apply for learning opportunities.
            </p>
            <button 
        ...
        <div className="col-span-2 bg-gray-100 p-6 rounded-lg shadow-md">
          <h1 className="text-4xl font-bold text-gray-900 mb-6">Post a Skill</h1>
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col mb-4">
              <label htmlFor="skills" className="text-gray-700 font-semibold mb-2">Having Skills</label>
              <textarea 
                id="skills" 
                rows="4" 
                value={havingSkills.join('\n')}
                onChange={(e) => setHavingSkills(e.target.value.split('\n'))}
                className="border border-gray-300 p-2 rounded-lg" 
                placeholder="List your current skills"
              />
            </div>
            <div className="flex flex-col mb-4">
              <label htmlFor="salary" className="text-gray-700 font-semibold mb-2">Salary Expectation</label>
              <input 
                id="salary" 
                type="text" 
                value={salary}
