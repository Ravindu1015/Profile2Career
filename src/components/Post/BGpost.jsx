...
function BGpost() {
  ...
  const navClass = theme === 'blue' ? 'bg-blue-800 text-white' : 'bg-gray-100 text-gray-800';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'bgpost'), {
        bgjobtitle: jobTitle,
        bgskills: skills,
        bgsalary: salary,
        bgmoreinfo: moreInfo,
        bgimage: image ? URL.createObjectURL(image) : '' // Convert image to a URL string
      });
